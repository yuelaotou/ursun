...
//		this.isRead = false; // Lazy content not yet read
	...
	_getInnerHtml: function() {
		...
		// node name
		var tooltip = ( this.data && typeof this.data.tooltip == "string" ) ? " title=" + this.data.tooltip.replace(/'/g,"\\\'").replace(/"/g,'\\\"').replace(/\</g,'&lt;').replace(/\>/g,'&gt;') + "" : "";
		res +=  "<a href='#' class='" + opts.classNames.title + "'" + tooltip + ">" + this.data.title.replace(/'/g,"\\\'").replace(/"/g,'\\\"').replace(/\</g,'&lt;').replace(/\>/g,'&gt;') + "</a>";
		return res;
	},
...
//				self.append(data);
				self.addChild(data[ajaxOptions.root], null);

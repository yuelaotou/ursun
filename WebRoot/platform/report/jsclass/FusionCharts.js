...
function creatCharts(chartType,width,hight,chartID,dataXML,divName){
	var url = "../wee/report/swf/"
	var swf_url = null;
	if(chartType == "COLUMN_2D"){
		swf_url = url+"Column2D.swf";//2D单柱状图
	}else if(chartType == "BAR_2D"){
		swf_url	= url+"Bar2D.swf";//2DBAR状图
	}else if(chartType=="MSBAR_2D"){
		swf_url = url+"MSBar2D.swf";
	}else if(chartType=="COLUMN_3D"){
		swf_url = url+"Column3D.swf";//3D柱状图
	}else if(chartType=="MSCOLUMN_3D"){
		swf_url = url+"MSColumn3D.swf";//3D多维多柱状图
	}else if(chartType=="MSBAR_3D"){
		swf_url = url+"MSBar3D.swf";//3D多BAR柱状图
	}else if(chartType=="COMBIN_BAR"){
		swf_url = url+"MSCombi3D.swf";//三维柱状图
	}else if(chartType=="PIE_2D"){
		swf_url = url+"Pie2D.swf";//2D饼图
	}else if(chartType=="PIE_3D"){
		swf_url = url+"Pie3D.swf";//3D饼图
	}else if(chartType=="LINE_2D"){
		swf_url = url+"Line.swf";//折线图
	}else if(chartType=="MSLINE_2D"){
		swf_url = url+"MSLine.swf";//二维多线折线图
	}else if(chartType=="LINE_3D"){
		swf_url = url+"MSCombi3D.swf";//3D折线图
	}else if(chartType=="STACKEDCOLUMN_2D"){
		swf_url = url+"StackedColumn2D.swf";//二维堆积Bar图
	}else if(chartType=="STACKEDBAR_2D"){
		swf_url = url+"StackedBar2D.swf";//2D堆积Bar图
	}else if(chartType=="STACKEDCOLUMN_3D"){
		swf_url = url+"StackedColumn3D.swf";//二维堆积Bar图
	}else if(chartType=="STACKEDBAR_3D"){
		swf_url = url+"StackedBar3D.swf";//3D堆积Bar图
	}else{
		swf_url = null;
	}
	var chartid = new FusionCharts(swf_url,chartID,width,hight,'0','1');
	if(dataXML!=null){
		chartid.setDataXML(dataXML);
	}
	chartid.render(divName);
}
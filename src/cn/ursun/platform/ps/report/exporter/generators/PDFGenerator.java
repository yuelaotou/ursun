/**
 * 文件名：PDFGenerator.java
 *
 * 创建人：【周洋】 - 【zhouyang@neusoft.com】
 *
 * 创建时间：Dec 23, 2009 1:58:39 PM
 *
 * 版权所有：东软软件股份有限公司
 */
...
public class PDFGenerator {
    ...
    {
        logger = Logger.getLogger(PDFGenerator.class.getName());
        pageIndex = 0;
        pagesData = new ArrayList<ExportBean>();
    ...
    private byte[] addImageToPDF(int id, boolean isCompressed)
    {
        ...
        {
            logger.severe((new StringBuilder("Exception while parsing image data for PDF: ")).append(e.toString()).toString());
        }
    ...
    private byte[] getBitmapData24(int id)
    {
        ...
        while(rows.hasMoreElements()) 
            for(pixels = new StringTokenizer((String)rows.nextElement(), ","); pixels.hasMoreElements();)
            {
                ...
                {
                    logger.severe((new StringBuilder("Exception while writing image data for PDF: ")).append(e.toString()).toString());
                }
            }
        ...
        {
            logger.severe((new StringBuilder("Exception while closing stream for PDF: ")).append(e.toString()).toString());
        }
    ...
    public byte[] getPDFObjects(boolean isCompressed)
    {
        ...
        {
            logger.severe((new StringBuilder("Exception while writing PDF data: ")).append(e.toString()).toString());
        }
        return pdfBytes;
    }
    ...
    private byte[] hexToBytes(char hex[])
    {
        ...
        for(int i = 0; i < length; i++)
        {
            ...
            raw[i] = (byte)((value & 255) > 127 ? value - 256 : value);
        }
    ...
    int pageIndex;
    ArrayList<ExportBean> pagesData;
}

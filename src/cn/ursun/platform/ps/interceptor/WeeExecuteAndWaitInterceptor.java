/**
 * 文件名：WeeExecuteAndWaitInterceptor.java
 * 
 * 创建人：段鹏 - duanp@neusoft.com
 * 
 * 创建时间：2009-3-24 下午02:20:25
 * 
 * 版权所有：东软集团股份有限公司
 */
...
/*
 * $Id: ExecuteAndWaitInterceptor.java 508914 2007-02-18 16:45:30Z tschneider $ Licensed to the Apache Software
 ...
 * responsible for issuing a subsequent request back to the action, giving the effect of a self-updating progress meter</b>.
 * 
 ...
 * top of the stack</b>. This allows you to display progress data, such as a count, in the wait page. By making the
 * wait page automatically reload the request to the action (which will be short-circuited by the interceptor), you can
 * give the appearance of an automatic progress meter.
 * 
 * <p/>This interceptor also supports using an initial wait delay. An initial delay is a time in milliseconds we let the
 * server wait before the wait page is shown to the user. During the wait this interceptor will wake every 100 millis to
 * check if the background process is done premature, thus if the job for some reason doesn't take to long the wait page
 * is not shown to the user. <br/> This is useful for e.g. search actions that have a wide span of execution time. Using
 * a delay time of 2000 millis we ensure the user is presented fast search results immediately and for the slow results
 * a wait page is used.
 * 
 * <p/><b>Important</b>: Because the action will be running in a seperate thread, you can't use ActionContext because
 * it is a ThreadLocal. This means if you need to access, for example, session data, you need to implement SessionAware
 ...
 * <li>delay (optional) - an initial delay in millis to wait before the wait page is shown (returning <code>wait</code>
 * as result code). Default is no initial delay.</li>
 * <li>delaySleepInterval (optional) - only used with delay. Used for waking up at certain intervals to check if the
 ...
 * </pre>
 * 
 * <p/> <u>Example code2:</u> This example will wait 2 second (2000 millis) before the wait page is shown to the user.
 * Therefore if the long process didn't last long anyway the user isn't shown a wait page.
 * 
 * <pre>
 * &lt;action name=&quot;someAction&quot; class=&quot;com.examples.SomeAction&quot;&gt;
 *     &lt;interceptor-ref name=&quot;completeStack&quot;/&gt;
 ...
 * </pre>
 * 
 * <p/> <u>Example code3:</u> This example will wait 1 second (1000 millis) before the wait page is shown to the user.
 * And at every 50 millis this interceptor will check if the background process is done, if so it will return before the
 * 1 second has elapsed, and the user isn't shown a wait page.
 * 
 * <pre>
 * &lt;action name=&quot;someAction&quot; class=&quot;com.examples.SomeAction&quot;&gt;
 *     &lt;interceptor-ref name=&quot;completeStack&quot;/&gt;
 ...
 * 
 */
public class WeeExecuteAndWaitInterceptor extends MethodFilterInterceptor {

	private static final long serialVersionUID = -2754639196749652512L;

	private static final Log LOG = LogFactory.getLog(WeeExecuteAndWaitInterceptor.class);
	 ...
	 */
	protected WeeBackgroundProcess getNewBackgroundProcess(String name, ActionInvocation actionInvocation,
			int threadPriority) {
		return new WeeBackgroundProcess(name + "BackgroundThread", actionInvocation, threadPriority);
	}

	/**
	 * Returns the name to associate the background process. Override to change the way background processes are mapped
	 * to requests.
	 * 
	 * @param actionInvocation
	 *            The action invocation
	...
	protected String doIntercept(ActionInvocation actionInvocation) throws Exception {
		...
		String cancel = ServletActionContext.getRequest().getParameter("weeExecAndWait.cancel");
		synchronized (session) {
			WeeBackgroundProcess bp = (WeeBackgroundProcess) session.get(KEY + name);
			if (cancel != null && cancel.equals("true")) {
				if (bp != null && !bp.isDone()) {// 已开启后台线程，并且正在运行中，终止它
					bp.destory();
					session.remove(KEY + name);
				}
				ServletActionContext.getResponse().getOutputStream().write(
						new String("{'weeExecAndWait.successCancel':'true'}").getBytes());
	...
	/**
	 * Performs the initial delay. <p/> When this interceptor is executed for the first time this methods handles any
	 * provided initial delay. An initial delay is a time in miliseconds we let the server wait before we continue.
	 * <br/> During the wait this interceptor will wake every 100 millis to check if the background process is done
	 * premature, thus if the job for some reason doesn't take to long the wait page is not shown to the user.
	 * 
	 * @param bp
	 ...
	 */
	protected void performInitialDelay(WeeBackgroundProcess bp) throws InterruptedException {
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Sleeping ended after " + step + " steps and the background process is "
					+ (bp.isDone() ? " done" : " not done"));
		}
	}
	 ...
	 * 
	 * @param threadPriority
	 *            the priority from <code>Thread.XXX</code>
	 */
	 ...
	 * 
	 * @param executeAfterValidationPass
